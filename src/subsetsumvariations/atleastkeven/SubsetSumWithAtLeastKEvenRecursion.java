...
public class SubsetSumWithAtLeastKEvenRecursion {
...
class Solution {
    ...
    public boolean find(int index,int[] nums,int target,int k,int evenCount){
        ...
        if(nums[index]<=target)
            pick = find(index+1,nums,target-nums[index],k,nums[index]%2==0?evenCount+1:evenCount);

        boolean notPick = find(index+1,nums,target,k,evenCount);
