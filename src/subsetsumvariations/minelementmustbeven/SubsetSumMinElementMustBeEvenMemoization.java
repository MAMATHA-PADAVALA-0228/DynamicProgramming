...
class Solution1 {
    ...
    public boolean find(int index, int[] nums, int target, int minPickedIndex, int[][][] dp) {
        ...
        if(minPickedIndex != -1 && dp[index][target][minPickedIndex]!=-1)
            return dp[index][target][minPickedIndex]==1;
        ...
        if(nums[index]<=target) {
            ...
            pick = find(index + 1, nums, target - nums[index], nums[index] <= nums[newIndex] ? index : newIndex, dp);
        }

        boolean notPick = find(index+1,nums,target,minPickedIndex,dp);

        if(minPickedIndex!=-1)
        dp[index][target][minPickedIndex] = pick || notPick?1:0;
    ...
    public boolean subsetSumMinEven(int[] nums, int target) {
        ...
        return find(0,nums,target,-1,dp);
    }

}

public class SubsetSumMinElementMustBeEvenMemoization {
