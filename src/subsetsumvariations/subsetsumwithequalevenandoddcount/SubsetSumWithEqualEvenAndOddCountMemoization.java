...
class Solution1 {
    ...
    public boolean find(int index,int[] nums,int target,int countEven,int countOdd,int[][][][] dp){
        ...
        if(dp[index][target][countEven][countOdd]!=-1)
            return dp[index][target][countEven][countOdd]==1;

        if(nums[index]<=target)
            pick = find(index+1,nums,target-nums[index],nums[index]%2==0?countEven+1:countEven,nums[index]%2!=0?countOdd+1:countOdd,dp);

        boolean notPick = find(index+1,nums,target,countEven,countOdd,dp);

        dp[index][target][countEven][countOdd] = pick || notPick ?1:0;
    ...
    public boolean subsetSumEqualEvenOdd(int[] nums, int target) {
        ...
        int[][][][] dp = new int[nums.length][target+1][target+1][target+1];
        ...
        return find(0,nums,target,0,0,dp);

    }

}

public class SubsetSumWithEqualEvenAndOddCountMemoization {
