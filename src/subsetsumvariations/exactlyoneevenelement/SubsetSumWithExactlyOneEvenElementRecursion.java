...
class Solution {
    ...
    public  boolean find(int index,int[] nums,int target,int count){
        ...
        if(nums[index]<=target)
            pick = find(index+1,nums,target-nums[index],nums[index]%2==0?count+1:count);

        boolean notPick = find(index+1,nums,target,count);
...
public class SubsetSumWithExactlyOneEvenElementRecursion {
