...
class Solution1 {
    ...
    public boolean find(int index,int[] nums,int target,boolean isPrevEven,int[][][] dp){

        if(index==nums.length) return target==0;

        if(dp[index][target][isPrevEven?1:0]!=-1)
            return dp[index][target][isPrevEven?1:0]==1;
        ...
        if(nums[index]<=target)
        {
            ...
            if(canPick)
                pick = find(index+1,nums,target-nums[index],nums[index]%2==0,dp);

        }

        boolean notPick = find(index+1,nums,target,isPrevEven,dp);

        dp[index][target][isPrevEven?1:0] = pick || notPick ? 1 : 0;
        return pick || notPick;
...
public class SubsetSumIfPreviousPickWasEvenNextPickMustBeOddMemoization {
