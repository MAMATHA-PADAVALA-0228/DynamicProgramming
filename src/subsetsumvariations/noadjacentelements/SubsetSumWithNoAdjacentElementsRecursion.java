...
class Solution {
    ...
    public boolean find(int index,int[] nums,int target,boolean isPrevTaken){
        ...
        if(nums[index]<=target && !isPrevTaken)
            pick = find(index+1,nums,target-nums[index],true);

        boolean notPick = find(index+1,nums,target,false);
...
public class SubsetSumWithNoAdjacentElementsRecursion {
