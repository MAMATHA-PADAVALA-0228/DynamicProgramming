...
class Solution {
    ...
    public boolean find(int[] nums,int target,int k,int index,int evenCount){
        ...
        if(nums[index]<=target)
            pick = find(nums,target-nums[index],k,index+1,nums[index]%2==0?evenCount+1:evenCount);

        boolean notPick = find(nums,target,k,index+1,evenCount);
    ...
    public boolean subsetSumAtMostKEven(int[] nums, int target, int k) {
        ...
        // Your code here
        return find(nums,target,k,0,0);

    }

}

public class SubsetSumWithAtMostKEvenRecursion {
