...
class Solution {
    ...
    public boolean find(int index,int[] nums,int target,int[] taken){
        ...
        if(nums[index]<=target) {
            if (index >= 2) {
                if (taken[index - 1] !=1 || taken[index - 2] != 1) {
                    taken[index] = 1;
                    pick = find(index + 1, nums, target - nums[index], taken);
                    taken[index] = 0;
                }
        ...
        boolean notPick = find(index+1,nums,target,taken);
        return  pick || notPick;
    }
    ...
    public boolean subsetSumNoThreeConsecutive(int[] nums, int target) {
        ...
        return find(0,nums,target,new int[nums.length]);
    }

}

public class SubsetSumWithoutThreeConsecutiveElementsRecursion {
